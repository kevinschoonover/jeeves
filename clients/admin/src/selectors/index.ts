...
const restaurantSelector = (state: AppState) => state.restaurants;

export const getMaterialChartItems = createSelector(
  materialItemsSelector,
  (items: any[]) => {
    const categories = _.groupBy(items, (x) => x.category);
    const data = _.keys(categories).map((category) => ({
      name: category,
...
export const getMailitems = createSelector(
  mailSelector,
  (mail: any) => {
    return _.sortBy(
      mail.items.map(
        (item: any) =>
          _.assign({}, item, { createdAt: moment(item.createdAt) }),
        (i: any) => i.createdAt
      )
    );
  }
